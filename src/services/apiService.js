 ...
 */
export const speechToText = async (audioBlob, sourceLanguage = 'en-IN') => {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Speech-to-text error: ${response.status} ${response.statusText}. Details: ${JSON.stringify(errorData)}`);
    }
 ...
 * @param {string} targetLanguage - Target language code (e.g., 'hi', 'bn', 'kn')
 * @returns {Promise<string>} - Translated text
 */
export const translateText = async (text, sourceLanguage = 'en-IN', targetLanguage = 'hi') => {
  try {
    // Validate input text
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(`Translation error: ${response.status} ${response.statusText}. Details: ${JSON.stringify(errorData)}`);
    }
 ...
 * @param {string} language - Language code (e.g., 'hi', 'bn', 'kn')
...
export const textToSpeech = async (text, language = 'hi') => {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(`Text-to-speech error: ${response.status} ${response.statusText}. Details: ${JSON.stringify(errorData)}`);
    }
